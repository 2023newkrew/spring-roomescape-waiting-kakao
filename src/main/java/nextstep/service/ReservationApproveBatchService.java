...
@Service
public class ReservationApproveBatchService {
    ...
    private void recoverFailedReservations(LocalDateTime start) {
        ...
        for (ReservationTransitionSnapshot reservationTransitionSnapshot : reservationTransitionSnapshots) {
            ...
            transactionUtil.executeTask(() -> write(status, reservationTransitionSnapshot.getReservationProjections()));
        }
    }

    private int transitReservationStatusByChunkSize(ReservationSearch reservationSearch) {
        return transactionUtil.executeTask(() -> {
            List<ReservationProjection> reservationProjections = read(reservationSearch);
            write(reservationSearch.getNextStatus(), reservationProjections);
    ...
    private List<ReservationProjection> read(ReservationSearch reservationSearch) {
        return reservationDao.findAllByStatusAndCreatedAt(reservationSearch);
    }

    private void write(ReservationStatus status, List<ReservationProjection> reservationProjections) {
        try {
