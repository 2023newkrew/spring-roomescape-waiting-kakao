...
public class MemberController {
    ...
    @PostMapping
    public ResponseEntity memberSave(@RequestBody MemberRequest memberRequest) {
        Long id = memberService.addMember(memberRequest);
        return ResponseEntity.created(URI.create("/members/" + id)).build();
    }

    @Operation(summary = "현재 로그인 된 자신의 정보 조회 API")
    @GetMapping("/me")
    public ResponseEntity memberDetails(@LoginMember UserDetails userDetails) {
